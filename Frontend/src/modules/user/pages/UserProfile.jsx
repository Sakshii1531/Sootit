...
const UserProfile = () => {
    ...
    const MenuButton = ({ icon: Icon, label, sublabel, onClick, color = "text-neutral-500", bg = "bg-neutral-50" }) => (
        <button 
            onClick={onClick}
            className="w-full flex items-center justify-between p-3.5 active:bg-neutral-50 transition-colors group"
        >
            <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col items-start">
                    <span className="text-[12px] font-black tracking-tight text-neutral-900">{label}</span>
                    {sublabel && <span className="text-[9px] font-bold text-neutral-400 uppercase tracking-widest">{sublabel}</span>}
                </div>
            </div>
            <ChevronRight size={14} className="text-neutral-300 group-hover:text-blue-600 transition-colors" strokeWidth={3} />
        </button>
    );
            ...
            <div className="px-6 pt-8 pb-6">
                ...
                <div className="flex items-center gap-4 mb-8">
                    <div className="relative">
                        <div className="h-20 w-20 bg-slate-800 rounded-[2rem] flex items-center justify-center text-white text-2xl font-black shadow-2xl shadow-slate-800/30 border-2 border-white">
                            {user.profile.name?.[0] || 'U'}
                        </div>
                        <div className="absolute -bottom-0.5 -right-0.5 h-6 w-6 bg-slate-900 rounded-full border-2 border-white flex items-center justify-center">
                    ...
                    <div className="flex-1">
                        ...
                        <span className="text-[9px] font-black text-neutral-400 uppercase tracking-[0.2em] mb-2 block">ID: USR-7721A-09</span>
                        <div className="w-fit inline-flex items-center gap-1 bg-slate-50 text-slate-900 px-2.5 py-1 rounded-lg border border-slate-100 shadow-sm">
                            <Star size={10} className="fill-slate-900" />
                            <span className="text-[8px] font-black uppercase tracking-widest leading-none">Premium User</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-10">
                    ...
                    <motion.button 
                        whileTap={{ scale: 0.98 }}
                        onClick={logoutUser}
                        className="w-full h-16 bg-slate-50 text-slate-900 rounded-[1.8rem] border border-slate-100 flex items-center justify-center gap-4 font-black uppercase text-[10px] tracking-[0.3em] active:scale-95 transition-all shadow-xl shadow-slate-900/5 mb-10"
                    >
